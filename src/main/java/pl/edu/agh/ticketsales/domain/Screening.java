...
// <basic getters&setters>
    ...
    public void setHallId(Hall hall) {
        ...
        if(seatsStatus != null && Arrays.asList(seatsStatus).contains(true)){
            int rows = Math.min(this.seatsStatus.length, hall.getRows());
            int rowLength = Math.min(this.seatsStatus[0].length, hall.getRowLength());
            ...
            for(int i= 0; i < rows; i++){
                for(int j =0; j < rowLength; j++){
                    this.seatsStatus[i][j] = tempSeats[i][j];
                }
            }
...
//    public void newSeats();
    public void setSeatStatus(int row, int seat, boolean status) {this.seatsStatus[row][seat] = status;}
